...
// Filter by field matching input value
  ...
  var filterData = data.filter(data => data.city === inputCity && data.state === inputState && data.country === inputCountry);
// Add filtered sighting to table
  ...
  if (response.filterData.length !== 0) {
    populate(filterData);
      ...
      || response.filterState.length !==0 || response.filterCountry.length !== 0 || response.filterShape.length !== 0 ))){
